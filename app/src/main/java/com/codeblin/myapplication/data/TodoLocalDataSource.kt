...
interface TodoLocalDataSource: ArchCoreDataSource<TodoItem>

class TodoLocalDataSourceImpl: TodoLocalDataSource {
    private var todos = listOf<TodoItem>()

    override suspend fun getAll(): List<TodoItem> {
        todos = listOf(
            TodoItem(1, "Task 1 from local", false),
            TodoItem(2, "Task 2 from local", false),
            TodoItem(3, "Task 3 from local", false),
        )
        return todos
    }
